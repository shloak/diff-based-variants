...
(function (TransformationType) {
    ...
    TransformationType[TransformationType["Replace"] = 3] = "Replace";
})(TransformationType || (TransformationType = {}));
function cartesianOrder(children) {
...
function setNodeValue(node, propKey, value) {
    switch (propKey) {
        case 'height':
        ...
        case 'width':
            ...
            break;
        case 'mainComponent':
            node['mainComponent'] = figma.getNodeById(value['id']);
            break;
        default:
            node[propKey] = value;
            break;
...
function applyTransformation(node, transformation, path = [0]) {
        ...
        if (transformation.transformationType == TransformationType.Modify) {
            ...
            switch (transformation.replacePayload['newType']) {
                ...
                case 'INSTANCE':
                    ...
                    newNode = figma.getNodeById(mainComponent['id']).createInstance();
...
function findTransformations(inputNode, outputNode) {
    ...
    let transformations = [];
    let q = [{ base: inputNode, compare: outputNode, path: [0], insideInstance: false }];
    while (q.length > 0) {
